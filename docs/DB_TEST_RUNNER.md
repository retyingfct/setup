...
Version `0.4.31-draft` contains all 400 management rows. Executable adapters are PostgreSQL 90/102, MySQL 90/107, MariaDB 75/91, and Oracle 80/100. Every other row is explicitly classified as environment-dependent, manual, or not applicable and is shown in console output; no row remains generic pending work. PostgreSQL CSV assertions are evaluated after standards-compliant CSV decoding. A13 caps a misbehaving setup wizard transcript at 64 KiB and enforces a short hard timeout. Clone runs can map a unique OS hostname to the copied collector Agent ID used as the receiver directory. MySQL-family SQL snapshots are headerless, and the temporary slow-log and JSON error-sink settings changed by D4b, D4c, and D7a are restored to their captured values. MySQL marker and Unicode queries preserve SQL comments and set `long_query_time=0` for their own connection only so the slow log records correlation markers without altering the server-global threshold. Constrained stability, outage, and soak checks use one-minute lab windows, with generated counts and result text derived from that duration. The MySQL-family G9 generator uses shell-safe query fragments, preserves its correlation comments, and sets a session-local zero slow-query threshold. MySQL D4b/D4c and MariaDB F5d/F5e select a disposable database before table operations, and G15 preserves generated marker comments. H2 evaluates recovered delivery independently from H1's disk-buffer-growth assertion. The `--scenario` option accepts a comma-separated list and preserves the requested execution order. MySQL D2f evaluates its own format scope against the complete correlated receiver block rather than inheriting D2 severity. MySQL D1b writes real include-file newlines under `/etc/mysql`, resets systemd's failure counter before restarting, edits the regular `mysql.cnf` target, and retains its backup until MySQL restarts successfully. A9 retries its freshly installed health endpoint for up to 15 seconds to avoid a service-start race. H6 reboot continuations use absolute evidence paths, systemd-safe format strings, and comment-preserving MySQL markers. D2b/D2c send a valid MySQL Protocol 4.1 rejection probe, H8 waits past systemd's activating state, and H9 generates its marker while RELP is unreachable. G13 journals, restarts, and verifies restoration of the original MySQL-family slow-log path. D2b/D2c use the official receiver-side MySQL 8 client and require `mysql-client-core-8.0` there; blocked-host setup still uses bounded handshake disconnects. D2c uses bounded receiver-side TCP resets to increment MySQL's host error counter before the official-client blocked-host attempt. D2c uses repeated official-client authentication failures against a valid receiver-host account before its blocked-host attempt. Setup automation treats `Client tag` as a required identity prompt, supplies the disposable lab tag, matches annotated receiver-host and required PAT prompts immediately at their prompt delimiters, and supplies a disposable non-secret placeholder when the temporary clone-only setup requires an auto-update PAT. Multi-engine setup verifies running inputs through the collector process's open database-log descriptors rather than relying on absent application journal messages. Implemented adapters are not test results: only an actual evidence run can mark a scenario Pass or Fail.
