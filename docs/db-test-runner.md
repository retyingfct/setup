...
Version `0.4.7-draft` contains all 400 management rows. Executable adapters are PostgreSQL 90/102, MySQL 90/107, MariaDB 75/91, and Oracle 80/100. Every other row is explicitly classified as environment-dependent, manual, or not applicable and is shown in console output; no row remains generic pending work. PostgreSQL CSV assertions are evaluated after standards-compliant CSV decoding. A13 caps a misbehaving setup wizard transcript at 64 KiB and enforces a short hard timeout. Clone runs can map a unique OS hostname to the copied collector Agent ID used as the receiver directory. Implemented adapters are not test results: only an actual evidence run can mark a scenario Pass or Fail.
